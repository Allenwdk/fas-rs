...
if [ ! -n "$mod_exist" ]; then
    ...
    jq '.alias.fas_rs = "/dev/fas_rs/mode"' $VTOOLSDIR/profile.json | jq '.schemes.powersave.game = []' | jq '.schemes.balance.game = []' | jq '.schemes.performance.game = []' | jq '.schemes.fast.game = []' | jq '.schemes.pedestal.game = []' > $PATCHDIR/tmp && cp -f $PATCHDIR/tmp $VTOOLSDIR/profile.json && chmod a+rwx $VTOOLSDIR/profile.json
    ...
    sed -e "s/_fas_rs_version_/$fas_rs_version/" -e "s/_fas_rs_codename_/$fas_rs_codename/" -e "s/_mod_version_/$mod_version/" -e "s/_mod_codename_/$mod_codename/" -e "s/_fas_rs_mod_author_/$fas_rs_mod_author_" -e "s/_fas_rs_author_/$fas_rs_author/" -e "s/_scene_version_/$scene_version/" -e "s/_scene_author_/$scene_author/" $VTOOLSDIR/manifest.json > $PATCHDIR/tmp && cp -f $PATCHDIR/tmp $VTOOLSDIR/manifest.json && chmod a+rwx $VTOOLSDIR/manifest.json
    rm -rf $PATCHDIR/tmp
